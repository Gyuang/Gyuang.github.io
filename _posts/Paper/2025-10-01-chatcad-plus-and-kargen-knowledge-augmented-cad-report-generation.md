...
- 특성 융합: 그래프-증강 질환 특성과 지역 영상 특징을 두 가지 방식으로 융합(예: 게이팅/어텐션 계열 설계)하여 질환 민감도를 높임.
...
- 정량 지표: BLEU/ROUGE/METEOR/CIDEr 등 일반적 보고 지표와 임상 유효성 지표를 함께 고려(논문 세부 수치는 원문 참고).
...
### 4.6 파이프라인 도식
- 아래 도식은 KARGEN의 입력(영상·지식 그래프)→특성 추출(비전 인코더·GCN)→융합(모달리티 단위 라우팅)→동결 LLM 보고 생성 흐름을 요약합니다.
