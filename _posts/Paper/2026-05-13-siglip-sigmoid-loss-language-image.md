...
excerpt: "Replacing CLIP's softmax with a per-pair sigmoid loss reaches 84.5% ImageNet 0-shot in 2 days on a g/14 SigLiT and 83.2% with SigLIP SO-400M, while exposing that contrastive batch sizes saturate at 32k and degrade past 98k."
categories:
  - Paper
  - Multimodal-Alignment
permalink: /paper/siglip-sigmoid-loss-language-image/
tags:
  - SigLIP
  - SigLiT
  - Sigmoid-Loss
...
- Replace CLIP's softmax InfoNCE — which forces a global all-gather over the full B×B similarity matrix — with a per-pair **sigmoid binary classification loss** plus a learnable bias `b` (init −10) that absorbs the heavy negative-prior imbalance. Each (image, text) pair becomes an independent positive/negative judgement, dropping memory from |B|² to a per-device b² chunk.
- A chunked device-permutation implementation replaces two all-gathers with D collective permutes, so SigLiT B/8 hits **79.7% ImageNet 0-shot in 1 day on 4 TPU-v4 chips** and SigLiT g/14 reaches **84.5% in 2 days**. Full SigLIP SO-400M/14 (729 patches) reaches **83.2% ImageNet 0-shot, 82.9% ObjectNet, 70.2/52.0 COCO I→T/T→I R@1** — beating EVA-CLIP-E (5B params) at every metric.
- The 1M-batch run finally settles the "bigger is better" dogma: contrastive performance **peaks around 32k and degrades past ~98k** (sigmoid 73.4 → 73.0 → 71.6 at 32k / 98k / 307k for 9B-seen SigLIP B/16).

## Motivation

Softmax-CLIP's row/column normalization forces every device in a data-parallel job to see the full set of embeddings, demanding an all-gather and a |B|² similarity matrix that becomes the dominant cost as batches scale. Naïve softmax is also numerically unstable — implementations subtract per-row max, which is a *second* full-batch pass. The community had assumed huge batches (≥64k) were essential for contrastive quality, locking image-text pre-training out of any group without hundreds of accelerators.

The paper asks whether a loss that decomposes additively over pairs can (a) match softmax quality, (b) free up memory for larger effective batches on fewer chips, and (c) let the authors actually probe the batch-size scaling curve up to 1M to test the large-batch dogma — three questions that softmax cannot cleanly answer because the optimization, the memory, and the engineering trick are tangled.
...
|---|---|---|---|---|
| C1 | Sigmoid loss outperforms softmax at small batches (<16k) | Fig. 2 left/middle; Table 5 (4k: 65.3 vs 63.8; 8k: 68.6 vs 66.6; 16k: 72.3 vs 71.7); Table 7 (5-seed σ ±0.1–0.2pp) | INet 0-shot (SigLiT, SigLIP) | ⭐⭐⭐ — multiple seeds, two pre-training setups, consistent monotone gap of 1.6–2.0pp at B∈{4k,8k,16k} for 9B SigLIP |
| C2 | Performance saturates around 32k batch and *degrades* beyond ~98k | Fig. 2 (all 3 panels); Table 5 (sigmoid 9B: 32k=73.4, 98k=73.0, 307k=71.6); Table 8 (SigLiT 1M=84.7 vs 256k=84.7) | INet 0-shot, XM3600 36-lang | ⭐⭐⭐ — repeated across SigLiT/SigLIP/mSigLIP; the 1M run is the unique smoking gun |
| C3 | Sigmoid is more memory-efficient → larger B on fewer chips | 4 TPU-v4 fits B=4096 sigmoid vs 2048 softmax for Base SigLIP | qualitative resource note in §4.2 | ⭐⭐ — concrete number but only one configuration; no peak-memory profiling table |
...
| C5 | SigLIP SO-400M is SOTA at its parameter count | Table 3: 83.2 INet vs EVA-CLIP-E (5B params) 82.0 | INet/v2/ReaL/ObjectNet/COCO | ⭐⭐⭐ — beats much larger models across 6 metrics |
| C6 | Sigmoid is more robust to label/data noise | Fig. 7, 5 corruption regimes, p up to 0.5 | INet 0-shot, M/16 model | ⭐⭐ — single model size, single dataset; consistent gap but not stress-tested across scales |
...
**Honest read.** The two core scientific claims (C1: sigmoid wins at small B; C2: scaling saturates at 32k) are extremely well-supported — three independent setups (SigLiT, SigLIP, mSigLIP) trace the same curve, and the 1M-batch run is unique evidence in the literature. C5 (SOTA at parameter count) is also clean. What's weaker:

- **No variance bars on the headline batch-size plots.** Table 7 reports σ≈0.1–0.2pp at one batch size only; given the 32k→98k drop is 0.4pp (sigmoid 73.4→73.0), some "saturation curve" wiggle is within noise.
- **C3 (memory) is anecdotal** — 4096 vs 2048 on 4 chips, no per-component memory accounting, no comparison vs gradient-checkpointed softmax.
...
- **Reproducibility:** WebLI is private. Models are released, but anyone who wants to *retrain* must use a substitute dataset.

## Method & Architecture

![SigLIP loss as 6 lines of pseudocode](/assets/images/paper/siglip/page_002.png)
*Algorithm 1: the entire SigLIP loss in 6 lines — L2-normalize, dot-product with learned `t` and `b`, log-sigmoid against ±1 labels. The whole contribution fits on a screen.*
...
![Chunked device-permutation: each device holds b images and b texts, accumulates b×b negative blocks via D collective permutes](/assets/images/paper/siglip/page_003.png)
*Figure 1: chunked device-permutation. With per-device batch $b = |B|/D$, device $d_i$ holds $b$ images + $b$ texts. Each device computes its local b×b block (positives + b−1 local negatives), then permutes its text chunk to the next device and accumulates b×b negative blocks; after D−1 permutes every pair has been scored. Memory peak per device is $b^2$ (not $|B|^2$); all-gathers are replaced by D collective-permutes.*

### 4. SigLiT vs SigLIP

- **SigLiT** uses a *frozen* pre-trained image tower and trains only the text tower. With precomputed image embeddings, compute is near-trivial — that is how 4 TPU-v4 chips suffice for a 1-day, 79.7% INet run.
...
SigLIP SO-400M beats EVA-CLIP-E (12.5x its parameter count) on INet, INet-v2, INet-ReaL, ObjectNet, and COCO I→T, and ties on COCO T→I.
...
*Figure 2: across SigLiT, SigLIP, and 36-lang mSigLIP, sigmoid leads softmax at small batches; the curves cross around 32k; both losses peak by ~32k and degrade past ~98k.*
...
The 1.6–2.0pp small-batch advantage is real and replicated across both SigLiT and SigLIP setups, reproducing the SigLiT Fig. 2 left panel. The 1M-batch SigLiT run (84.7) merely *ties* the 256k run — a clean falsification of the large-batch dogma.

### Negative-ratio masking (Fig. 6, SigLiT B=16k, 900M)

Random masking of negatives degrades; keeping only **hard** negatives preserves quality almost fully; "easy only" collapses. So the ~268M negatives in a 16k batch are almost all uninformative — an efficient hard-negative miner could in principle match full-batch quality.

### Noise robustness (Fig. 7, M/16 at B=16384, 3.6B examples)

Sigmoid stays above softmax across image-corruption, text-corruption, batch-misalignment, and combined corruptions up to p=0.4–0.5. The gap widens as corruption increases.

### Bias-term ablation (Table 4)

| Config | INet 0-shot | Pet | CIFAR-100 |
|---|---|---|---|
| **Default (b=−10, t'=log 10)** | **63.0** | – | – |
| No bias | 62.0 | – | – |
| b=0, t'=log 1 (pathological) | 53.7 | – | – |

The −10 init is worth ~1.0pp on INet over no-bias and ~9pp over pathological init.
...
- **Reproducibility** — WebLI is private; only LiT (also Google) is public among pre-training corpora.
...
The SO-400M shape — 400M params, 729 patches, 83.2 INet — is also the reference visual encoder shape that subsequent multimodal LLMs (PaLI-3, downstream LLaVA-style stacks) adopted, so SigLIP weights have outlived the SigLIP paper as a default vision tower.
