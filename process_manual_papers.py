#!/usr/bin/env python3
"""
Process Manually Configured Papers

This script processes papers with manually configured ArXiv IDs
"""
...
class ManualPaperProcessor:
    def __init__(self, base_dir: str = "/Users/gyu/Desktop/Gyuang.github.io"):
        ...
        # Load manual configurations
        self.manual_papers = self.load_manual_configs()
        
        # Ensure directories exist
        self.images_dir.mkdir(parents=True, exist_ok=True)

    def load_manual_configs(self) -> Dict:
        """Load manual paper configurations"""
        manual_file = self.base_dir / "manual_papers.json"
        try:
            with open(manual_file, 'r', encoding='utf-8') as f:
                return json.load(f)
        except FileNotFoundError:
            logger.error(f"Manual config file not found: {manual_file}")
            return {}
    ...
    def process_paper(self, paper_name: str, config: Dict) -> bool:
        ...
        # Insert into post
        self.insert_images_into_post(config['post_file'], saved_images)
        
        logger.info(f"Successfully processed {paper_name}")
        return True

    def process_all_manual_papers(self) -> Dict[str, bool]:
        """Process all manually configured papers"""
        results = {}
        
        logger.info(f"Processing {len(self.manual_papers)} manually configured papers")
        
        for paper_name, config in self.manual_papers.items():
            try:
    ...
    def create_summary_report(self, results: Dict[str, bool]) -> str:
        ...
        report = []
        report.append("# Manual Papers Processing Report\n")
        ...
        if results:
            ...
            for paper_name, success in results.items():
                ...
                config = self.manual_papers.get(paper_name, {})
                arxiv_id = config.get('arxiv_id', 'Unknown')
                
                report.append(f"### {paper_name}")
                report.append(f"- **Status**: {status}")
                report.append(f"- **ArXiv ID**: {arxiv_id}")
                report.append(f"- **Post File**: {config.get('post_file', 'Unknown')}")
                report.append("")
...
def main():
    ...
    logger.info("Starting Manual Papers Processing")
    
    processor = ManualPaperProcessor()
    
    # Process all manual papers
    results = processor.process_all_manual_papers()
    
    # Generate report
    report = processor.create_summary_report(results)
    
    # Save report
    report_path = processor.base_dir / "manual_papers_report.md"
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(report)
    
    logger.info(f"Manual processing complete. Report saved to: {report_path}")
    
    # Print summary
    successful = sum(results.values())
    total = len(results)
    print(f"\n{'='*60}")
    print(f"MANUAL PAPERS PROCESSING SUMMARY")
    print(f"{'='*60}")
